...
- **微信公众号草稿箱**：走微信官方 API，支持多账号、封面 / 摘要、素材库封面、草稿关联更新、账号级发布默认值和可选的标题 AI 润色。
...
5.  **🚀 一键同步到微信草稿箱 (v2.2 增强)**
    ...
    - **标题 AI 润色（可选）**：发布窗口的文章标题旁可一键让 LLM 根据正文优化标题（给 5 个候选），并自动移除标题里的 `(MM-DD-HHMM)` 时间码。可在设置里开关、选择模型质量（DeepSeek V4 Pro / Lite）。
...
### AI 功能（AI Provider · AI 编排 · 标题润色）

- **统一 Provider**：设置里的 `AI Provider` 是所有 AI 能力的共享凭证源。配置一次（当前主推 DeepSeek，`Base URL` 默认 `https://api.deepseek.com/v1`，填 API Key，模型选 `DeepSeek V4`），选为「默认 AI Provider」即可。**AI 编排**和**标题 AI 润色**都复用这个 Provider 的凭证。
- **同级功能各自成组**（互不干扰）：
  - **AI 编排（排版）**：在设置的「AI 编排（排版）」折叠区里，有独立的 *启用开关* 和 *模型质量下拉*（DeepSeek V4 Pro / Lite），以及默认布局、默认颜色、参考图片、超时、缓存等。生成入口在转换器顶部工具栏的 `AI 编排` 按钮。
  - **标题 AI 润色**：在设置的「标题 AI 润色」区里，有独立的 *启用开关*（关闭后发布窗口不再显示润色按钮）和 *模型质量下拉*。
- **模型质量在消费方选**：Provider 层只标明模型家族 `DeepSeek V4`；具体 Pro（质量优先）/ Lite（快省）由 AI 编排、标题润色各自在自己的区块里选。
- Provider 底层仍支持 3 类接口（OpenAI 兼容 / Gemini / Anthropic），DeepSeek 走 OpenAI 兼容。
- 内置 3 套布局家族：
