...
只需一键，即可将您的 Markdown 笔记转换为符合微信生态美学、阅读体验极佳的 HTML；也可以在发布窗口选择其他平台，通过 Wechatsync 浏览器扩展保存为各平台草稿。无论是代码块、引用、列表、本地图片还是公式图表，都尽量保持从 Obsidian 到发布端的完整呈现。
...
现在，Wechat Converter 不再只服务于微信公众号。你可以在同一个“发布与分发”窗口里继续把文章发送到知乎、掘金、CSDN、语雀、小红书等平台，由 Wechatsync 浏览器扩展接管真实的浏览器登录态和草稿保存流程。

- **微信仍走官方 API**：公众号草稿箱继续使用插件自己的 AppID / AppSecret 同步链路，保留封面、摘要、多账号和发布默认值。
- **其他平台走浏览器扩展**：知乎、掘金、CSDN 等平台通过 Wechatsync 使用浏览器登录态保存为草稿，不需要在 Obsidian 里重复登录每个平台。
- **发布前可选平台**：在发布弹窗中切换到“其他平台”，勾选目标平台后发送到浏览器扩展。
- **轻量状态提示**：设置页和发布窗口会显示桥接连接状态、已选平台和上次检测到的登录状态，最终结果以浏览器扩展任务窗口为准。
- **适合多渠道创作者**：同一篇 Obsidian 笔记可以先发公众号，再同步到多个内容平台草稿箱，最后分别检查排版并手动发布。

> 多平台分发仍是 Beta 能力。各平台的草稿保存、风控、登录状态和失败重试由 Wechatsync 浏览器扩展处理；Obsidian 侧负责写作、排版、平台选择和任务投递。
...
1.  **🌐 多平台发布 (Multi-platform Publishing) ⭐ Beta**
    - **一处写作，多处分发**：在 Obsidian 中完成写作和排版后，可继续发送到知乎、掘金、CSDN、语雀、小红书等平台草稿箱。
    - **Wechatsync 浏览器扩展接管平台登录态**：其他平台通过浏览器已有登录态保存草稿，Obsidian 不内嵌平台登录，也不接管 Cookie。
    - **微信与其他平台分工清晰**：微信公众号继续使用插件自己的官方 API 链路，其他平台走 Wechatsync 扩展链路。
    - **发布窗口直接选择平台**：在“发布与分发”窗口切换到“其他平台”，勾选平台后发送到浏览器扩展。
    - **任务结果回到扩展查看**：草稿链接、失败原因和单平台重试由 Wechatsync 任务窗口处理，减少 Obsidian 里的等待和阻塞。
...
- **多平台发布进入发布窗口**：现在“发布与分发”窗口支持切换到“其他平台”，把文章发送到 Wechatsync 浏览器扩展，再由扩展保存到知乎、掘金、CSDN 等平台草稿箱。
- **Wechatsync 桥接设置更完整**：设置页新增多平台分发配置、Token 校验、平台选择、连接测试和已选平台登录状态检测。
...
5. **发送到其他内容平台** ⭐ Beta
   - 先安装并登录 Wechatsync 浏览器扩展，确保目标平台已经在浏览器里登录。
   - 在插件设置里开启 **多平台分发（Beta）**，填入 Wechatsync 扩展 `CLI / MCP 连接` 中显示的地址、端口和 Token。
   - 点击“测试连接”，确认 Obsidian 能连接到浏览器扩展。
   - 在设置页勾选想同步的平台；需要时可点击“检测已选平台登录状态”。
   - 回到转换器，点击 **发布与分发**，切换到 **其他平台**。
   - 勾选知乎、掘金、CSDN 等目标平台后，点击 **发送到浏览器扩展**。
   - Obsidian 会把当前文章投递给 Wechatsync；后续草稿链接、失败原因和单平台重试请在浏览器扩展任务窗口中查看。
