...
- Phase 9(设置面板 UI，需求驱动·为后续大量迭代设置 UI 做准备): 抽出转换视图内「设置面板」11 方法到独立目录 `views/settings-panel/settings-panel.js`(mixin)：createSettingsPanel(~390行)/createSection/resetSettingsPanelViewState + 7 个 onXxxChange 变更处理器 + updateButtonActive。**架构决策**：`togglePanel` 是设置面板与 AI 面板**共用**的通用 view 助手，保留在 input.js 不随迁(避免 feature 模块间隐性耦合)；`showAccountSetupEmptyState` 归属同步弹窗(wechat-sync-modal 在调)，也不迁。**顺带解耦**：`getImageSwipeCommandCopy`/`createImageSwipeCalloutMarkdown` 等 image-swipe 纯函数群从 input.js 下沉到新 `services/image-swipe.js`；共用常量 `APPLE_STYLE_VIEW_TITLE` 升到 services/settings-defaults.js。input.js 3199→2581。898/898 绿、scan 65 文件绿。
